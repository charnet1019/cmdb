"""Tests for import_service.generate_category_template's required-field markers."""
...
def test_gpt_create_template_marks_credentials_as_required():
    buffer, _filename = import_service.generate_category_template("gpt", "create")
...
def test_gpt_create_template_required_fields_all_have_asterisk():
    for field_name, label, required in import_service.GPT_CREATE_FIELDS:
        if required:
