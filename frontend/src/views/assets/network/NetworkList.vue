...
import { useTypeTree, categories, platformOptions, categoryOptions, deviceTypeOptions } from '../composables/useTypeTree'
...
const form = ref({
  name: '',
  asset_code: '',
  category: 'network' as AssetCategory,
  address: '',
  platform: '',
  device_type: '',
  model: '',
...
const currentCategory = 'network' as AssetCategory
          ...
          <div class="text-sm">
            ...
            <template v-else>
              <template v-for="node in flattenedTypeTree" :key="node.id">
                <div v-if="node.category === 'network' || node.isRoot" class="py-1.5 px-2 rounded cursor-pointer hover:bg-slate-50 flex items-center gap-1" :class="isSelectedTypeNode(node) ? 'bg-primary/10 text-primary' : 'text-slate-600'" :style="{ paddingLeft: `${node.level * 16 + 8}px` }" @click="handleSelectType(node)">
                  ...
                  <component :is="getCategoryIcon(node.category || 'network')" class="text-sm mr-1" />
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden relative">
          <div class="overflow-x-auto">
            <table class="data-table min-w-[800px]">
              <thead><tr><th class="w-10"><input type="checkbox" class="rounded border-gray-300 w-3.5 h-3.5" @change="selectAllChanged($event)" :checked="allSelected" /></th><th>名称</th><th>地址</th><th>设备类型</th><th>厂商/型号</th><th>用户名密码</th><th class="text-right">操作</th></tr></thead>
              <tbody>
                <tr v-if="assets.length === 0 && !loading"><td colspan="7" class="text-center py-16 text-slate-400">暂无数据</td></tr>
                <template v-else>
                  <tr v-for="asset in assets" :key="asset.id" :class="{ 'opacity-50 bg-slate-50': !asset.is_active }">
                    ...
                    <td><span class="text-sm text-slate-600">{{ asset.device_type || '-' }}</span></td>
                    <td><span class="text-sm text-slate-600">{{ asset.platform && asset.model ? `${asset.platform}/${asset.model}` : (asset.platform || asset.model || '-') }}</span></td>
                    ...
                    <td class="text-right"><button v-if="asset.is_active" @click="openEditModal(asset)" class="bg-primary text-white px-2 py-0.5 rounded text-xs">更新</button><button v-else class="bg-slate-200 text-slate-400 px-2 py-0.5 rounded cursor-not-allowed text-xs">更新</button><button v-if="asset.is_active" @click="handleDelete(asset, fetchData)" class="border border-red-400 text-red-500 px-2 py-0.5 rounded text-xs ml-1">删除</button></td>
                  </tr>
                </template>
              </tbody>
            </table>
