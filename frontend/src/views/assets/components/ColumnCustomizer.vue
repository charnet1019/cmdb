  ...
  <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white w-full max-w-md rounded-xl shadow-2xl">
      <div class="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
        <h2 class="text-base font-semibold text-slate-900">自定义列</h2>
        <button @click="close" class="p-1 hover:bg-slate-100 rounded">
      ...
      <div class="p-4">
        ...
        <div class="mb-3 pb-3 border-b border-slate-100">
          <div class="text-xs text-slate-400 mb-2">固定显示</div>
          <div class="grid grid-cols-3 gap-2">
            <div
              v-for="col in fixedColumns"
              :key="col.key"
              class="flex items-center gap-2 px-2.5 py-1.5 bg-slate-50 rounded"
            >
              <input
                type="checkbox"
                checked
                disabled
                class="rounded border-gray-300 w-3.5 h-3.5 cursor-not-allowed"
              />
              <span class="text-xs text-slate-500">{{ col.label }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-2">
          <div
            ...
            :key="col.key"
            class="flex items-center gap-2 px-2.5 py-1.5 rounded hover:bg-slate-50 transition-colors cursor-pointer"
          >
            <input
              :id="`col-${col.key}`"
              type="checkbox"
            ...
            />
            <label :for="`col-${col.key}`" class="text-xs text-slate-700 cursor-pointer select-none">{{ col.label }}</label>
          </div>
        </div>
        ...
        <div class="flex justify-end gap-2 mt-4 pt-3 border-t border-slate-100">
          ...
          <button
            @click="close"
            class="px-3 py-1.5 text-xs bg-primary text-white rounded hover:bg-primary-container"
          >
            确定
          </button>
        </div>
