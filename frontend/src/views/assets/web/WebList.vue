...
import { useTypeTree, categories } from '../composables/useTypeTree'
...
const form = ref({ name: '', asset_code: '', category: 'web' as AssetCategory, address: '', platform: '', url: '', notes: '' })
const currentCategory = 'web' as AssetCategory
...
function openCreateModal() {
  ...
  form.value = { name: '', asset_code: '', category: 'web', address: '', platform: '', url: '', notes: '' }
          ...
          <div class="text-sm">
            ...
            <template v-else>
              <template v-for="node in flattenedTypeTree" :key="node.id">
                <div v-if="node.category === 'web' || node.isRoot" class="py-1.5 px-2 rounded cursor-pointer hover:bg-slate-50 flex items-center gap-1" :class="isSelectedTypeNode(node) ? 'bg-primary/10 text-primary' : 'text-slate-600'" :style="{ paddingLeft: `${node.level * 16 + 8}px` }" @click="handleSelectType(node)">
                  <UpOutlined v-if="node.isRoot && expandedTypeIds.includes('all')" class="text-xs" @click.stop="toggleType('all')" /><DownOutlined v-else-if="node.isRoot && !expandedTypeIds.includes('all')" class="text-xs" @click.stop="toggleType('all')" /><UpOutlined v-else-if="node.level === 1 && node.hasChildren && expandedTypeIds.includes(node.id)" class="text-xs" @click.stop="toggleType(node.id)" /><DownOutlined v-else-if="node.level === 1 && node.hasChildren && !expandedTypeIds.includes(node.id)" class="text-xs" @click.stop="toggleType(node.id)" /><RightOutlined v-else-if="node.level === 2" class="text-xs" /><span v-else-if="node.level === 1 && !node.hasChildren" class="w-4"></span><component :is="getCategoryIcon(node.category || 'web')" class="text-sm mr-1" /><span class="flex-1 truncate">{{ node.name }}</span>
                </div>
              </template>
            </template>
          </div>
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden relative">
          <div class="overflow-x-auto">
            <table class="data-table min-w-[800px]">
              <thead><tr><th class="w-10"><input type="checkbox" class="rounded border-gray-300 w-3.5 h-3.5" @change="selectAllChanged($event)" :checked="allSelected" /></th><th>名称</th><th>URL</th><th>用户名密码</th><th class="text-right">操作</th></tr></thead>
              <tbody>
                <tr v-if="assets.length === 0 && !loading"><td colspan="5" class="text-center py-16 text-slate-400">暂无数据</td></tr>
                <template v-else>
