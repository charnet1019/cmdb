              ...
              <tr v-for="asset in assets" :key="asset.id" :class="{ 'opacity-50': !asset.is_active }">
                ...
                <td>
                  <div v-for="cred in asset.credentials || []" :key="cred.id" class="flex items-center gap-1.5 text-slate-600 py-1">
                    ...
                    <EyeOutlined v-if="asset.is_active && !decryptedPasswords.has(cred.id)" class="text-[14px] cursor-pointer hover:text-primary ml-1" @click="viewPassword(cred)" />
                    <EyeInvisibleOutlined v-if="asset.is_active && decryptedPasswords.has(cred.id)" class="text-[14px] cursor-pointer hover:text-primary ml-1" @click="viewPassword(cred)" />
                  </div>
                </td>
                <td class="text-right">
