...
import {
  DownOutlined, UpOutlined, RightOutlined, CloseOutlined, PlusCircleOutlined,
  EyeOutlined, EyeInvisibleOutlined, CopyOutlined, StopOutlined, CheckCircleOutlined,
  DeleteOutlined, FolderOutlined, FolderOpenOutlined, FileTextOutlined,
...
const form = ref({ name: '', asset_code: '', category: 'database' as AssetCategory, address: '', platform: '', notes: '' })
const currentCategory = 'database' as AssetCategory
...
function openCreateModal() {
  ...
  form.value = { name: '', asset_code: '', category: 'database', address: '', platform: '', notes: '' }
...
function openEditModal(asset: Asset) {
  ...
  form.value = { name: asset.name, asset_code: asset.asset_code || '', category: asset.category, address: asset.address || '', platform: asset.platform || '', notes: asset.notes || '' }
          ...
          <div class="text-sm">
            ...
            <template v-else>
              <template v-for="node in flattenedTypeTree" :key="node.id">
                <div v-if="node.category === 'database' || node.isRoot" class="py-1.5 px-2 rounded cursor-pointer hover:bg-slate-50 flex items-center gap-1" :class="isSelectedTypeNode(node) ? 'bg-primary/10 text-primary' : 'text-slate-600'" :style="{ paddingLeft: `${node.level * 16 + 8}px` }" @click="handleSelectType(node)">
                  <UpOutlined v-if="node.isRoot && expandedTypeIds.includes('all')" class="text-xs" @click.stop="toggleType('all')" /><DownOutlined v-else-if="node.isRoot && !expandedTypeIds.includes('all')" class="text-xs" @click.stop="toggleType('all')" /><UpOutlined v-else-if="node.level === 1 && node.hasChildren && expandedTypeIds.includes(node.id)" class="text-xs" @click.stop="toggleType(node.id)" /><DownOutlined v-else-if="node.level === 1 && node.hasChildren && !expandedTypeIds.includes(node.id)" class="text-xs" @click.stop="toggleType(node.id)" /><RightOutlined v-else-if="node.level === 2" class="text-xs" /><span v-else-if="node.level === 1 && !node.hasChildren" class="w-4"></span><component :is="getCategoryIcon(node.category || 'database')" class="text-sm mr-1" /><span class="flex-1 truncate">{{ node.name }}</span>
                </div>
              </template>
            </template>
          </div>
      ...
      <div class="relative bg-white w-full max-w-2xl rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-6">
          <form @submit.prevent="handleSubmit" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-medium text-slate-600 mb-1.5">资产类型</label><select v-model="form.category" class="input-field"><option v-for="cat in categoryOptions" :key="cat.key" :value="cat.key">{{ cat.label }}</option></select></div><div><label class="block text-xs font-medium text-slate-600 mb-1.5">数据库类型</label><select v-model="form.platform" class="input-field"><option value="">请选择</option><option v-for="p in platformOptions[form.category]" :key="p" :value="p">{{ p }}</option></select></div></div>
