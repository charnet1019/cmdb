...
function openCreateModal() {
  ...
  form.value = { name: '', asset_code: '', category: 'host', address: '', platform: '', device_type: '', model: '', serial_number: '', url: '', notes: '' }
  if (selectedTypeNode.value?.subCategory) {
    form.value.platform = selectedTypeNode.value.subCategory
  }
  resetFormCredentials()
...
function validateAddress(address: string): { valid: boolean; message?: string } {
  if (!address?.trim()) return { valid: false, message: '地址不能为空' }
  const parts = address.split(':')
  if (parts.length > 2) return { valid: false, message: '格式错误，端口只能有一个' }
  const host = parts[0]
  const port = parts[1]
  const ipRegex = /^(\d{1,3}\.){3}\d{1,3}$/
  const hostnameRegex = /^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/
  if (!ipRegex.test(host) && !hostnameRegex.test(host)) return { valid: false, message: '无效的IP地址或主机名' }
  if (port) {
    const portNum = parseInt(port, 10)
    if (isNaN(portNum) || portNum < 1 || portNum > 65535) return { valid: false, message: '端口号必须在 1-65535 之间' }
  }
  return { valid: true }
}
...
async function handleSubmit() {
  ...
  if (form.value.address) {
    const validation = validateAddress(form.value.address)
    if (!validation.valid) { message.error(validation.message); return }
  }
    ...
    if (editingAsset.value) {
      ...
      for (const cred of formCredentials.value) {
        if (cred.username && cred.password) {
          if (cred.id) {
            await fetch('/api/v1/credentials/' + cred.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: cred.username, password: cred.password }) })
          } else {
            await fetch('/api/v1/credentials?asset_id=' + editingAsset.value.id, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: cred.username, password: cred.password, credential_type: 'password' }) })
          }
        }
      }
      ...
      for (const cred of formCredentials.value) {
        if (cred.username && cred.password && response.id) {
          await fetch('/api/v1/credentials?asset_id=' + response.id, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: cred.username, password: cred.password, credential_type: 'password' }) })
        }
      }
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-x-auto">
      <div class="flex border-b border-slate-100">
        <router-link v-for="cat in categories" :key="cat.key" :to="cat.key === 'all' ? '/assets' : `/assets/${cat.key}`" class="flex items-center gap-2 px-6 py-4 text-sm font-medium whitespace-nowrap" :class="(cat.key === 'all' && $route.path === '/assets') || $route.path === `/assets/${cat.key}` ? 'text-teal-600 border-b-2 border-teal-600' : 'text-slate-500 hover:text-slate-700'">
          <component :is="cat.icon" class="text-lg" />{{ cat.label }}
        </router-link>
      </div>
    </div>
          ...
          <div class="text-sm">
            <template v-if="treeViewMode === 'asset'">
              ...
              <template v-if="isRootExpanded">
                <div v-for="org in flattenedOrgs" :key="org.id" class="py-1.5 px-2 rounded cursor-pointer hover:bg-slate-50 flex items-center gap-1" :class="{'bg-primary/10 text-primary': selectedOrgId === org.id, 'text-slate-600': selectedOrgId !== org.id, 'opacity-50': draggedOrgId === org.id, 'bg-blue-50 border-t-2 border-primary': dragOverOrgId === org.id}" :style="{ paddingLeft: `${(org.level + 1) * 20 + 8}px` }" @click.stop="handleOrgClick(org)" @contextmenu.prevent.stop="handleOrgContextMenu($event, { id: org.id, name: org.name, isRoot: false })" draggable="true" @dragstart="handleOrgDragStart($event, org.id)" @dragover.prevent="handleOrgDragOver($event, org.id)" @dragleave="handleOrgDragLeave" @drop.prevent="handleOrgDrop($event, org.id)" @dragend="handleOrgDragEnd">
                  <DownOutlined v-if="org.hasChildren && isOrgExpanded(org.id)" class="text-xs" /><RightOutlined v-else-if="org.hasChildren && !isOrgExpanded(org.id)" class="text-xs" /><span v-else class="w-3"></span>
                  <FolderOutlined v-if="org.hasChildren && !isOrgExpanded(org.id)" class="text-sm" /><FolderOpenOutlined v-else-if="org.hasChildren && isOrgExpanded(org.id)" class="text-sm" /><FileTextOutlined v-else class="text-sm text-slate-400" />
                  <span class="flex-1 truncate">{{ org.name }}</span><span class="text-xs text-slate-400">({{ org.count }})</span>
                </div>
              </template>
            ...
            <template v-else>
              <template v-for="node in flattenedTypeTree" :key="node.id">
                <div v-if="node.category === 'host' || node.isRoot" class="py-1.5 px-2 rounded cursor-pointer hover:bg-slate-50 flex items-center gap-1" :class="isSelectedTypeNode(node) ? 'bg-primary/10 text-primary' : 'text-slate-600'" :style="{ paddingLeft: `${node.level * 16 + 8}px` }" @click="handleSelectType(node)">
                  <UpOutlined v-if="node.isRoot && expandedTypeIds.includes('all')" class="text-xs" @click.stop="toggleType('all')" />
                  <DownOutlined v-else-if="node.isRoot && !expandedTypeIds.includes('all')" class="text-xs" @click.stop="toggleType('all')" />
                  <UpOutlined v-else-if="node.level === 1 && node.hasChildren && expandedTypeIds.includes(node.id)" class="text-xs" @click.stop="toggleType(node.id)" />
                  <DownOutlined v-else-if="node.level === 1 && node.hasChildren && !expandedTypeIds.includes(node.id)" class="text-xs" @click.stop="toggleType(node.id)" />
                  <RightOutlined v-else-if="node.level === 2" class="text-xs" />
                  <span v-else-if="node.level === 1 && !node.hasChildren" class="w-4"></span>
                  <component :is="getCategoryIcon(node.category || 'host')" class="text-sm mr-1" />
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <Dropdown :trigger="['click']">
                <button class="border border-slate-300 text-slate-600 text-xs px-3 py-1.5 rounded hover:bg-slate-50 flex items-center gap-1">更多操作 <DownOutlined class="text-[10px]" /></button>
                <template #overlay>
                  <div class="bg-white rounded-lg shadow-lg border border-slate-200 py-1 min-w-[140px]">
                    <div @click="canDisable && bulkDisable(fetchData)" class="px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 cursor-pointer flex items-center gap-2" :class="!canDisable ? 'opacity-50 cursor-not-allowed' : ''"><StopOutlined class="text-sm" />批量禁用<span v-if="selectedActiveCount > 0" class="text-xs text-slate-400 ml-auto">({{ selectedActiveCount }})</span></div>
                    <div @click="canActivate && bulkActivate(fetchData)" class="px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 cursor-pointer flex items-center gap-2" :class="!canActivate ? 'opacity-50 cursor-not-allowed' : ''"><CheckCircleOutlined class="text-sm" />批量激活<span v-if="selectedInactiveCount > 0" class="text-xs text-slate-400 ml-auto">({{ selectedInactiveCount }})</span></div>
                    <div class="border-t border-slate-100 my-1"></div>
                    <div @click="selectedCount > 0 && bulkDelete(fetchData)" class="px-4 py-2 text-sm text-red-500 hover:bg-red-50 cursor-pointer flex items-center gap-2"><DeleteOutlined class="text-sm" />批量删除<span v-if="selectedCount > 0" class="text-xs text-slate-400 ml-auto">({{ selectedCount }})</span></div>
                  </div>
                </template>
              </Dropdown>
            </div>
              ...
              <tbody>
                ...
                <template v-else>
                  <tr v-for="asset in assets" :key="asset.id" :class="{ 'opacity-50 bg-slate-50': !asset.is_active }">
                    ...
                    <td class="text-right"><button v-if="asset.is_active" @click="openEditModal(asset)" class="bg-primary text-white px-2 py-0.5 rounded text-xs">更新</button><button v-else disabled class="bg-slate-200 text-slate-400 px-2 py-0.5 rounded cursor-not-allowed text-xs">更新</button><button v-if="asset.is_active" @click="handleDelete(asset, fetchData)" class="border border-red-400 text-red-500 px-2 py-0.5 rounded text-xs ml-1">删除</button><button v-else disabled class="border border-slate-200 text-slate-300 px-2 py-0.5 rounded cursor-not-allowed text-xs ml-1">删除</button></td>
                  </tr>
                </template>
              </tbody>
            </table>
      ...
      <div class="relative bg-white w-full max-w-2xl rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-6">
          <form @submit.prevent="handleSubmit" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-medium text-slate-600 mb-1.5">型号</label><input v-model="form.model" type="text" class="input-field" placeholder="设备型号" /></div><div><label class="block text-xs font-medium text-slate-600 mb-1.5">序列号</label><input v-model="form.serial_number" type="text" class="input-field" placeholder="序列号" /></div></div>
