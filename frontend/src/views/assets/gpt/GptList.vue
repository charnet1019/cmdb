...
const form = ref({ name: '', asset_code: '', category: 'gpt' as AssetCategory, address: '', platform: '', url: '', notes: '' })
const currentCategory = 'gpt' as AssetCategory
...
function openCreateModal() {
  ...
  form.value = { name: '', asset_code: '', category: 'gpt', address: '', platform: '', url: '', notes: '' }
          ...
          <div class="text-sm">
            ...
            <template v-else>
              <template v-for="node in flattenedTypeTree" :key="node.id">
                <div v-if="node.category === 'gpt' || node.isRoot" class="py-1.5 px-2 rounded cursor-pointer hover:bg-slate-50 flex items-center gap-1" :class="isSelectedTypeNode(node) ? 'bg-primary/10 text-primary' : 'text-slate-600'" :style="{ paddingLeft: `${node.level * 16 + 8}px` }" @click="handleSelectType(node)">
                  <UpOutlined v-if="node.isRoot && expandedTypeIds.includes('all')" class="text-xs" @click.stop="toggleType('all')" /><DownOutlined v-else-if="node.isRoot && !expandedTypeIds.includes('all')" class="text-xs" @click.stop="toggleType('all')" /><UpOutlined v-else-if="node.level === 1 && node.hasChildren && expandedTypeIds.includes(node.id)" class="text-xs" @click.stop="toggleType(node.id)" /><DownOutlined v-else-if="node.level === 1 && node.hasChildren && !expandedTypeIds.includes(node.id)" class="text-xs" @click.stop="toggleType(node.id)" /><RightOutlined v-else-if="node.level === 2" class="text-xs" /><span v-else-if="node.level === 1 && !node.hasChildren" class="w-4"></span><component :is="getCategoryIcon(node.category || 'gpt')" class="text-sm mr-1" /><span class="flex-1 truncate">{{ node.name }}</span>
                </div>
              </template>
            </template>
          </div>
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden relative">
          <div class="overflow-x-auto">
            <table class="data-table min-w-[800px]">
              <thead><tr><th class="w-10"><input type="checkbox" class="rounded border-gray-300 w-3.5 h-3.5" @change="selectAllChanged($event)" :checked="allSelected" /></th><th>名称</th><th>平台</th><th>API端点</th><th>API Key</th><th class="text-right">操作</th></tr></thead>
              <tbody>
                ...
                <template v-else>
                  <tr v-for="asset in assets" :key="asset.id" :class="{ 'opacity-50 bg-slate-50': !asset.is_active }">
                    ...
                    <td><span class="text-sm text-slate-600 font-mono">{{ asset.url || '-' }}</span></td>
      ...
      <div class="relative bg-white w-full max-w-2xl rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-6">
          <form @submit.prevent="handleSubmit" class="space-y-5">
            ...
            <div><label class="block text-xs font-medium text-slate-600 mb-1.5">API端点</label><input v-model="form.url" type="text" class="input-field" placeholder="https://api.openai.com" /></div>
            <div class="border border-slate-200 rounded-lg overflow-hidden">
              <div class="bg-slate-50 px-4 py-2.5 border-b border-slate-200"><label class="text-xs font-medium text-slate-600">API Key</label></div>
              <div class="p-4">
                <div v-if="formCredentials.length > 0" class="space-y-2 mb-4"><div v-for="(cred, index) in formCredentials" :key="index" class="bg-slate-50 px-3 py-2 rounded-lg border border-slate-200"><div class="flex items-center gap-3"><div class="flex items-center gap-2 flex-1"><span class="text-xs text-slate-500">API Key:</span><span class="font-medium text-slate-700">{{ cred.username }}</span></div><div class="flex items-center gap-2 flex-1"><span class="text-xs text-slate-500">密码:</span><span class="text-slate-400 font-mono">********</span></div><button type="button" @click="removeCredentialFromForm(index)" class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded"><DeleteOutlined class="text-sm" /></button></div></div></div>
                <div class="flex items-end gap-3"><div class="flex-1"><label class="block text-xs text-slate-500 mb-1">API Key 名称</label><input v-model="newCredentialForm.username" type="text" class="input-field text-sm" placeholder="如: OpenAI API Key" /></div><div class="flex-1 relative"><label class="block text-xs text-slate-500 mb-1">密钥</label><input v-model="newCredentialForm.password" :type="showPassword ? 'text' : 'password'" class="input-field text-sm w-full pr-8" placeholder="输入密钥" /><button type="button" @click="showPassword = !showPassword" class="absolute right-2 top-[calc(50%+10px)] -translate-y-1/2 text-slate-400 hover:text-slate-600"><EyeOutlined v-if="!showPassword" class="text-sm" /><EyeInvisibleOutlined v-else class="text-sm" /></button></div><button type="button" @click="addCredentialToForm" class="bg-primary text-white px-4 py-2 rounded text-sm hover:bg-blue-600 h-[38px]"><PlusCircleOutlined class="mr-1" /> 添加</button></div>
              </div>
            </div>
